...
const ArtisanProfile = () => {
  ...
  const [artisanInfo, setArtisanInfo] = useState<
    (typeof profileData)[0] | null
  >(null);
  const fetchArtisanData = async () => {
    const artisanData =
      profileData.find((artisan) => artisan.id === Number(artisanId)) ?? null;
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex flex-col sm:items-end">
                <span className="text-sm text-gray-600">
                  Jobs Completed: {artisanInfo.jobsCompleted}
                </span>
          ...
          <div className="">
            ...
            >
              {profileData.slice(0, 8).map((profile) => (
                <SwiperSlide className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300 border group cursor-pointer flex flex-col items-center text-center">
                  {/* Profile Image */}

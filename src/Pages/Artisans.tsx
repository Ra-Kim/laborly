...
const Artisans = () => {
  ...
  const categories = artisanCategories?.map((data) => data.title);
  ...
  const filterArtisanCategory = (category: string) => {
    setSelectedCategory(category);
    let filtered =
      category === "All"
        ? profileData
        : profileData.filter((data) => data.role === category);
  ...
  const filterArtisanState = (state: string) => {
    setSelectedState(state);
    let filteredItems =
      state === ""
        ? profileData
        : profileData.filter((data) => data.location === state);
        ...
        <div className="py-10 grid grid-cols-1 md:grid-cols-4 xl:grid-cols-5  gap-5 lg:gap-8">
          ...
          <div className="bg-white rounded-xl p-3 md:p-6 soft-shadow md:col-span-1">
            <div className="hidden md:block">
              <h3
                className="text-center text-lg text-darkPrimary cursor-pointer"
                onClick={() => {
                  setSelectedCategory("");
                  filterArtisanCategory("All");
                }}
              >
                All Categories
              </h3>
              ...
              <ul className="flex flex-col mt-10 gap-4">
                {categories.map((data, index) => (
                  <li
                    onClick={() => filterArtisanCategory(data)}
                    key={index}
                    className={`${
                      selectedCategory === data ? "bg-primary text-white" : ""
                    } cursor-pointer list-none text-[0.8rem]  p-2 hover:bg-primary hover:text-white rounded-md transition`}
                  >
                    {data}
                  </li>
                ))}
            ...
            <div className="flex items-center justify-between md:hidden mb-4 px-2">
              <h3 className="text-[1.3rem] font-semibold text-darkPrimary">
                Select Category
              </h3>
              <div
                onClick={toggleModal}
                className="w-10 h-10 flex items-center justify-center rounded-xl bg-white text-primary bg-opacity-20 shadow-xl duration-500 transition-all hover:bg-primary hover:text-white"
              >
              ...
              <div className="bg-white w-11/12 max-w-sm rounded-lg soft-shadow p-4">
                ...
                <ul className="max-h-60 overflow-y-auto text-sm">
                  {categories.map((data, index) => (
                    <li
                      onClick={() => {
                        filterArtisanCategory(data);
                        setShowModal(false);
                      }}
                      key={index}
                      className="cursor-pointer list-none text-[0.8rem]  p-2 hover:bg-primary hover:text-white rounded-md transition"
                    >
                      {data}
                    </li>
                  ))}
              ...
              {filterItems.slice(0, 6).map((profile) => (
                ...
                >
                  ...
                  <div className="border-t-2 border-b-2 py-2">
                    <p className="text-sm ">
                      {profile.shortBio.slice(0, 50)}...{" "}
                    </p>
                  </div>

                  {/* Skills */}
                  <div className="w-full mb-4">
                    <h5 className="text-sm font-medium text-gray-700 group-hover:text-primary my-2">
                      Skills
                    </h5>
